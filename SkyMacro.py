    ...
    while not stop_event.is_set():
        ...
        if peak <= 0.01:
            if silence_start is None:
                ...
                print("❗ No sound detected for 7 seconds!")
                stop_event.set()
                break
            else:
...
def macro_loop(stop_event):
    A_counter = 0
    while not stop_event.is_set():
        ad_random = random.uniform(120.5, 121)
        ...
        if A_counter == 3:
            ...
            A_counter = 0
            time.sleep(5)
            w_warp_random = random.uniform(0.2,0.5)
            hold_key(hwnd, VK_W, SC_W, w_warp_random, stop_event)
            hold_key(hwnd, VK_2, SC_2, 0.2, stop_event)
            time.sleep(0.6)
            hold_key(hwnd, VK_1, SC_1, 0.1, stop_event)
            continue

        w_random = random.uniform(1.5, 1.8)
